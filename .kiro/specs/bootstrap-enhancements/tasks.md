...
- [x] 1. Create tag module and property tests
  - [x] 1.1 Create `src/tags.ts` with `getStandardTags` function and `MANAGED_BY_TAG_VALUE` constant
  ...
  - [x] 1.2 Write property test for tag generation correctness
  ...
  - [x] 1.3 Write property test for empty purpose rejection
    - **Property 2: Empty purpose string is rejected**
    - Verify `getStandardTags("")` throws an Error
    - **Validates: Requirements 4.5**

- [x] 2. Add tagging to remote bootstrap
  - [x] 2.1 Apply Standard_Tag_Set to S3 bucket in `src/commands/remote.ts`
  ...
  - [x] 2.2 Apply Standard_Tag_Set to IAM role in `src/commands/remote.ts`
  ...
  - [x] 2.3 Apply Standard_Tag_Set to Lambda function in `src/commands/remote.ts`
  ...
  - [x] 2.4 Write unit tests for remote bootstrap tagging
...
- [x] 3. Add tagging to local bootstrap
  - [x] 3.1 Apply Standard_Tag_Set to Graveyard OU in `src/commands/bootstrap.ts`
    - Import `getStandardTags` from `../tags.js`
  ...
  - [x] 3.2 Write unit tests for local bootstrap tagging
...
- [x] 5. Implement permission set creation during remote bootstrap
  - [x] 5.1 Implement `ensureOrganizationManagementPermissionSet` in `src/commands/remote.ts`
  ...
  - [x] 5.2 Implement `ensureOrganizationRemoteManagementPermissionSet` in `src/commands/remote.ts`
  ...
  - [x] 5.3 Wire permission set creation into `runRemoteBootstrap`
  ...
  - [x] 5.4 Write unit tests for permission set creation
...
- [x] 6. Implement automatic type regeneration after remote apply
  - [x] 6.1 Implement `regenerateTypesFromState` helper in `src/commands/remote.ts`
  ...
  - [x] 6.2 Call `regenerateTypesFromState` after successful apply in `runRemoteApply`
    - After `Applied N operation(s)` log, call regeneration with `response.state`
    - _Requirements: 6.1, 6.2_

  - [x] 6.3 Write unit tests for post-apply regeneration
...
- [x] 8. Resolve TODO comments in `src/lambda/handler.ts`
  - [x] 8.1 Remove `as const` on literal booleans and `getStateUrl` response
  ...
  - [x] 8.2 Pass AWS clients through props in handler functions
  ...
  - [x] 8.3 Parallelize scan calls with `Promise.all` in `handleScan`
    - Replace sequential `scanOrganization` + `scanIdentityCenter` with `Promise.all([scanOrganization(...), scanIdentityCenter(...)])`
    - _Requirements: 7.1, 7.3, 7.4_

  - [x] 8.4 Use `assertUnreachable` pattern in action routing switch
...
- [x] 9. Resolve TODO comments in `src/commands/remote.ts`
  - [x] 9.1 Infer `RemoteCommandInput` type from valibot schema
  ...
  - [x] 9.2 Extract lambda zip reading to a helper function
  ...
  - [x] 9.3 Pass AWS clients as props in remote command functions
  ...
  - [x] 9.4 Remove `as any` cast on `CreateBucketCommand` input
  ...
  - [x] 9.5 Parallelize context/config loading with `Promise.all` in `runRemotePlan` and `runRemoteApply`
    - Replace sequential `readAwsContextFromFile` + `loadAwsConfigModelFromTsFile` with `Promise.all`
    - _Requirements: 7.1, 7.3, 7.4_

  - [x] 9.6 Use `assertUnreachable` pattern for operation formatting in `formatOperationLine`
    - Replace the final `const _exhaustive: never = operation` with a call to `assertUnreachable(operation)`
    - _Requirements: 7.1, 7.3, 7.4_

  - [x] 9.7 Do not extract properties from props in `ensureLambdaFunction`
    - Remove the destructuring `const { lambdaClient, roleArn, ... } = props` and use `props.*` directly
    - _Requirements: 7.1, 7.3, 7.4_

- [x] 10. Resolve TODO comments in `src/lambdaClient.ts`
  - [x] 10.1 Refactor `let rawResponse` pattern to avoid `let`
    - Use early-return or IIFE pattern to eliminate the `let rawResponse: InvokeCommandOutput` declaration
    - _Requirements: 7.1, 7.3, 7.4_

  - [x] 10.2 Refactor `let parsed` pattern to avoid `let`
    - Use early-return or IIFE pattern to eliminate the `let parsed: unknown` declaration
    - _Requirements: 7.1, 7.3, 7.4_

  - [x] 10.3 Use `assertUnreachable` pattern for error kind mapping
    - Replace the `switch` on `errorKind` with if/else + `assertUnreachable`
    - _Requirements: 7.1, 7.3, 7.4_

- [x] 11. Resolve TODO comments in `src/cli.ts` and `src/awsConfig.ts`
  - [x] 11.1 Use `assertUnreachable` pattern for remote subcommands in `src/cli.ts`
    - Replace the `switch (remoteSubcommand)` with if/else + `assertUnreachable`
    - _Requirements: 7.1, 7.3, 7.4_

  - [x] 11.2 Remove unnecessary `await` in `readAwsContextFromFile` in `src/awsConfig.ts`
