 ...
 * present in the cached IAM action catalog.
...
function renderIamPolicySupportTs(): string {
  return `${renderIamActionHintSupportTs()}
const nonEmptyPolicyStringSchema = v.pipe(v.string(), v.nonEmpty());
...
function renderIamActionHintSupportTs(): string {
  const serializedCatalog = JSON.stringify(iamActionCatalog, null, 2);
  const serviceHelpers = Object.keys(iamActionCatalog)
    .map(
      (servicePrefix) =>
        `  ${JSON.stringify(servicePrefix)}: (action: IamPolicyActionNameByService<${JSON.stringify(servicePrefix)}>) => iamAction(${JSON.stringify(servicePrefix)}, action),`,
    )
    .join("\n");

  return `const iamActionCatalog = ${serializedCatalog} as const;

export const iamActionCatalogSourceUrl = ${JSON.stringify(iamActionCatalogSourceUrl)};
export const iamActionCatalogSourceSha256 = ${JSON.stringify(iamActionCatalogSourceSha256)};

export type IamPolicyServicePrefix = keyof typeof iamActionCatalog;
export type IamPolicyActionNameByService<
  TService extends IamPolicyServicePrefix,
> = (typeof iamActionCatalog)[TService][number];
export type IamPolicyActionForService<
  TService extends IamPolicyServicePrefix,
