...
// --- Tests: Missing dist-lambda/lambda.zip ---

test("runRemoteBootstrap throws error when dist-lambda/lambda.zip is missing", async () => {
  ...
  try {
    ...
    // Run from workspace where dist-lambda/lambda.zip does not exist
    const originalCwd = process.cwd();
    process.chdir(workspace.workspacePath);
    try {
      await assert.rejects(
        () => runRemoteBootstrap(input),
        (error: Error) => {
          assert.ok(error.message.includes("dist-lambda/lambda.zip not found"));
          assert.ok(error.message.includes("npm run build:lambda"));
          return true;
        },
      );
    } finally {
...
test("runRemoteUpgrade throws error when dist-lambda/lambda.zip is missing", async () => {
  const workspace = await createTestWorkspace({ prefix: "remote-test-" });
  try {
